...
class AddAppointment extends React.Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:8000/ordered_users_with_location').then(resp => {
  ...
  handleChange = (event) => {
    if (this.state.ordered_users !== undefined) {
      const user = this.state.ordered_users.data.find(i => i.user_id === Number(event.target.value));
      this.setState({ alt: user.alt });
      this.setState({ lng: user.lng });
    }
  };
              ...
              <form className="ui form attached fluid segment" method="post" href="/schedule">
                ...
                <div className="equal width fields">
                  <div className="field">
                    <label>User</label>
                    {users !== undefined ?
                      <select id="id" name="id" onChange={this.handleChange}>
                        <option disabled selected value>Select user... </option>
                        {users.map(it =>
                          <option key={it.user_id} value={it.user_id}>{it.name} {it.surname}</option>
                        )}
                      </select>
                      : <Loading />
                    }
                ...
                <input type="hidden" id="alt" name="alt" value={this.state.alt} />
                <input type="hidden" id="lng" name="lng" value={this.state.lng} />
