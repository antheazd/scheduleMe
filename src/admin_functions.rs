...
pub async fn adminprofile(
    ...
    match cookies.get_private("id") {
        ...
        None => return Err(Redirect::to(uri!(adminlogin()))),
    }
}
    ...
    match id_cookie {
        Some(id) => {
            ...
            let users_query = sqlx::query(
                r#"SELECT users.id AS users_id, users.name, users.surname FROM users;"#,
            )
            .fetch_all(&mut *db)
            .await
            .unwrap();
            ...
            return Ok(Template::render(
                "adminpanel",
                context! {appointments: appointments, users: users},
            ));
        }
        _ => Err(Redirect::to(uri!(adminlogin()))),
    }
}
...
pub async fn adminchats(
    ...
    match id_cookie {
        Some(id) => {
            ...
            return Ok(Template::render(
                "adminchats",
                context! {users: users, other_users: other_users},
            ));
        }
        _ => Err(Redirect::to(uri!(adminlogin()))),
    }
...
pub async fn adminchat(
    ...
    match id_cookie {
        Some(id) => {
            ...
            let query =
                sqlx::query(r#"SELECT * FROM messages WHERE admin_id = $1 AND user_id = $2"#)
                    .bind(admin_id)
                    .bind(user_id)
                    .fetch_all(&mut *db)
                    .await
...
pub async fn adminpayments(
    ...
    match id_cookie {
        Some(id) => {
            ...
            let users_query = sqlx::query(
                r#"SELECT id, name, surname FROM users ORDER BY name ASC, surname ASC;"#,
            )
            .fetch_all(&mut *db)
            .await
            .unwrap();
...
pub async fn user_payments(
    ...
    match id_cookie {
        Some(id) => {
            ...
            return Ok(Template::render(
                "adminpayment",
                context! {appointments: appointments, user_info: user_info},
            ));
        }
        _ => Err(Redirect::to(uri!(adminlogin()))),
    }
