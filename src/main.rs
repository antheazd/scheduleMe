...
fn rocket() -> _ {
    rocket::build()
    .attach(Logs::init())
    .mount("/", routes![index, userprofile, userprofilepost, login, logout, signup, signupfn, loginfn, payments, add_payment, chat, add_user_message, schedule, add_appointment, indexget, adminpanel, adminpanelget,  adminlogin, adminloginfn, adminlogout, adminprofile, adminchats, adminchat, add_admin_message, adminpayments, location_get, location_post])
