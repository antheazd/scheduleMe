...
pub async fn profilepost(
    ...
    match id_cookie {
        Some(id) => {
            ...
            let update_user =
                sqlx::query(r#"select count(*) as locations from locations where user_id = $1;"#)
                    .bind(user_id)
                    .fetch_one(&mut *db)
                    .await;
        ...
        None => return Err(Template::render("profile", context! {})),
    }
}
    ...
    match id_cookie {
        Some(id) => {
            ...
            return Ok(Template::render(
                "schedule",
                context! {appointments: appointments, coordinates: coordinates},
            ));
        }
        _ => Err(Redirect::to(uri!(login()))),
    }
    ...
    match id_cookie {
        Some(id) => {
            ...
            let update_appointment =
                sqlx::query(r#"UPDATE appointments SET paid = true WHERE id = $1;"#)
                    .bind(appointment_id)
                    .fetch_one(&mut *db)
                    .await;
            ...
            Ok(_fine) => {
                ...
                match get_id {
                    Ok(id_result) => {
                        ...
                        create_cookies(id, name, surname, email, cookies);
                        return Ok(Redirect::to(uri!(location_get)));
                    }
                    Err(e) => {
                        println!("{e}")
                    }
